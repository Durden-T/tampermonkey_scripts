...
// @version      1.0.0
// @author       cyqu
// @description  Aggregate multiple paginated pages of lzt.market LoL listings and sort by price-per-champion (cost-performance).
...
(function() {
  'use strict';
	...
	function loadNumber(key, fallback) {
		if (hasGm()) {
			const raw = GM_getValue(NS + key, fallback);
	...
	function formatRatio(value, symbol = "") {
		if (!Number.isFinite(value) || value <= 0) return "—";
		if (value >= 1e3) return `${symbol}${Math.round(value).toLocaleString()}`;
		if (value >= 100) return `${symbol}${value.toFixed(1)}`;
		return `${symbol}${value.toFixed(2)}`;
	}
	var CARD_SELECTOR = ".marketIndexItem";
	var LIST_CONTAINER_SELECTOR = ".marketIndex--itemsContainer form.InlineModForm";
	var CHAMP_RE = /(\d+)\s+(characters?|champions?|чемпион|персонаж)/i;
	var CURRENCY_MAP = {
		rub: "₽",
		uah: "₴",
		kzt: "₸",
	...
	function findChampCount(card) {
		for (const group of card.querySelectorAll(".marketIndexItem--Badges")) {
			if (!group.querySelector(".badgeIc-lol")) continue;
			for (const badge of group.querySelectorAll(".marketIndexItem-Badge")) {
	...
	function parsePrice(card) {
		const priceEl = card.querySelector(".marketIndexItem--Price .Value");
	...
	function extractCardData(card) {
		...
		const ratio = price !== null && champCount > 0 ? price / champCount : Number.POSITIVE_INFINITY;
		return {
			el: card,
			id: card.id,
			price,
	...
	function detectCurrencySymbol(scope = document) {
		const icon = scope.querySelector(".marketIndexItem--Price [class*=\"svgIcon--\"]");
		if (!icon) return CURRENCY_FALLBACK;
		const code = Array.from(icon.classList).find((c) => c.startsWith("svgIcon--"))?.replace("svgIcon--", "").toLowerCase() ?? "";
		if (!code) return CURRENCY_FALLBACK;
		return CURRENCY_MAP[code] ?? code.toUpperCase();
	}
	var RATIO_BADGE_CLASS = "lzt-cps-ratio-badge";
	function decorateCard(card, data, symbol) {
		...
		meta.textContent = hasData ? `· ${data.champCount} champ${data.champCount === 1 ? "" : "s"}` : "· no champion data";
		badge.append(key, value, meta);
		if (!hasData) badge.dataset.empty = "true";
		card.appendChild(badge);
	}
	function stripDecoration(card) {
		card.querySelector(`.${RATIO_BADGE_CLASS}`)?.remove();
	}
	var RETRY_DELAYS_MS = [
		1e3,
	...
	async function fetchPages(pages, concurrency, signal, onProgress) {
		const total = pages.length;
		let done = 0;
		const results = new Array(total).fill(null);
		const failedPages = [];
		const queue = pages.map((p, idx) => ({
			page: p,
			idx
		}));
	...
	var DATA_HTML_URL_RE = /^\s*data:text\/html/i;
	function sanitizeElement(root) {
		...
		for (const el of elements) for (const attr of Array.from(el.attributes)) {
			...
			if (URL_ATTRS.has(name) && (JS_URL_RE.test(attr.value) || DATA_HTML_URL_RE.test(attr.value))) el.removeAttribute(attr.name);
		}
		return root;
	}
	...
	var snapshotContainerRef = null;
	function getContainer() {
		return document.querySelector(LIST_CONTAINER_SELECTOR);
	}
	function pageNavRoot(container) {
		return container.closest(".marketIndex--itemsContainer") ?? container.parentElement ?? document;
	}
	function setPageNavHidden(container, hidden) {
		for (const nav of pageNavRoot(container).querySelectorAll(".PageNav")) if (hidden) nav.setAttribute("hidden", "");
		else nav.removeAttribute("hidden");
	}
	function hasOriginalSnapshot() {
	...
	function snapshotOriginal(container) {
	...
	function computeStats(cards) {
		const ratios = cards.filter((c) => c.champCount > 0 && Number.isFinite(c.ratio)).map((c) => c.ratio).sort((a, b) => a - b);
		...
		return {
			best,
			median: ratios.length % 2 === 0 ? (ratios[mid - 1] + ratios[mid]) / 2 : ratios[mid],
			worst
		};
	}
	...
	function cleanupDecorations() {
		document.querySelectorAll(".marketIndexItem").forEach(stripDecoration);
	}
	_css("@import \"https://fonts.googleapis.com/css2?family=Instrument+Serif:ital@0;1&family=JetBrains+Mono:wght@400;500;700&display=swap\";#lzt-cps-mount{width:100%;margin:0;padding:0}.lzt-cps-panel{--cps-bg:#0c0c0f;--cps-bg-deep:#06060a;--cps-surface:#15151b;--cps-surface-2:#1d1d24;--cps-line:#28282f;--cps-line-bright:#3a3a44;--cps-ink:#f4efe4;--cps-ink-dim:#8a857a;--cps-ink-fade:#4d4a44;--cps-accent:#ffd60a;--cps-accent-deep:#b59500;--cps-info:#6ec3c9;--cps-danger:#ff6363;--cps-good:#9be15d;--cps-mono:\"JetBrains Mono\", ui-monospace, \"SF Mono\", Menlo, monospace;--cps-serif:\"Instrument Serif\", \"Times New Roman\", serif;all:revert;box-sizing:border-box;border:1px solid var(--cps-line);width:100%;color:var(--cps-ink);font-family:var(--cps-mono);letter-spacing:.02em;clip-path:polygon(0 0,calc(100% - 18px) 0,100% 18px,100% 100%,0 100%);background:repeating-linear-gradient(0deg,#ffffff04 0 1px,#0000 1px 3px),radial-gradient(circle at 6% -5%,#ffd60a14,#0000 38%),radial-gradient(circle at 105% 110%,#6ec3c90b,#0000 40%),#0c0c0f;margin:12px 0 14px;padding:18px 24px 16px;font-size:12px;line-height:1.45;animation:.42s cubic-bezier(.2,.8,.2,1) both cps-mount;display:block;position:relative}.lzt-cps-panel *{box-sizing:border-box;font-family:inherit}@keyframes cps-mount{0%{opacity:0;transform:translateY(-6px)}to{opacity:1;transform:translateY(0)}}.lzt-cps-panel:before,.lzt-cps-panel:after{content:\"\";border:1px solid var(--cps-accent);pointer-events:none;width:14px;height:14px;position:absolute}.lzt-cps-panel:before{border-bottom:none;border-right:none;top:6px;left:6px}.lzt-cps-panel:after{border-top:none;border-left:none;bottom:6px;right:6px}.cps-header{border-bottom:1px dashed var(--cps-line);justify-content:space-between;align-items:baseline;gap:16px;margin-bottom:14px;padding-bottom:12px;display:flex}.cps-header-title{align-items:baseline;gap:10px;display:flex}.cps-header-mark{font-family:var(--cps-mono);letter-spacing:.22em;color:var(--cps-accent);text-transform:uppercase;font-size:12px;font-weight:700}.cps-header-rule{background:var(--cps-line-bright);width:18px;height:1px;display:inline-block;transform:translateY(-3px)}.cps-header-name{font-family:var(--cps-serif);color:var(--cps-ink);letter-spacing:.005em;font-size:22px;font-style:italic;font-weight:400;line-height:1}.cps-header-meta{letter-spacing:.18em;text-transform:uppercase;color:var(--cps-ink-dim);align-items:center;gap:14px;font-size:10.5px;display:flex}.cps-header-mod{color:var(--cps-ink-dim)}.cps-header-mod b{color:var(--cps-ink);font-weight:500}.cps-header-clock{font-variant-numeric:tabular-nums;letter-spacing:.12em;color:var(--cps-ink)}.cps-controls{grid-template-columns:auto auto 1fr auto;align-items:stretch;gap:18px;display:grid}@media (width<=760px){.cps-controls{grid-template-columns:1fr 1fr;gap:14px}}.cps-stepper{flex-direction:column;gap:6px;min-width:158px;display:flex}.cps-stepper-label{letter-spacing:.28em;color:var(--cps-ink-dim);text-transform:uppercase;font-size:10px;font-weight:500}.cps-stepper-body{background:var(--cps-surface);border:1px solid var(--cps-line-bright);grid-template-columns:32px 1fr 32px;align-items:stretch;height:46px;display:grid}.cps-step{appearance:none;border:none;border-right:1px solid var(--cps-line);color:var(--cps-ink-dim);font-family:var(--cps-mono);cursor:pointer;background:0 0;justify-content:center;align-items:center;margin:0;padding:0;font-size:14px;transition:background .12s,color .12s;display:inline-flex}.cps-step:last-child{border-right:none;border-left:1px solid var(--cps-line)}.cps-step:hover:not(:disabled){color:var(--cps-accent);background:#ffd60a14}.cps-step:active:not(:disabled){color:var(--cps-accent);background:#ffd60a29}.cps-step:disabled{color:var(--cps-ink-fade);cursor:not-allowed}.cps-stepper-input{appearance:textfield;-webkit-appearance:none;-moz-appearance:textfield;width:100%;color:var(--cps-ink);font-family:var(--cps-mono);text-align:center;font-variant-numeric:tabular-nums;letter-spacing:.04em;background:0 0;border:none;outline:none;padding:0 6px;font-size:22px;font-weight:700}.cps-stepper-input::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}.cps-stepper-input::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}.cps-stepper-input:focus{color:var(--cps-accent)}.cps-stepper-meta{letter-spacing:.22em;text-transform:uppercase;color:var(--cps-ink-dim);justify-content:space-between;gap:12px;font-size:10px;display:flex}.cps-stepper-meta>:last-child{color:var(--cps-ink-fade)}.cps-stepper-meta[data-warn=true]{color:var(--cps-danger)}.cps-stepper-meta[data-warn=true]>:last-child{color:var(--cps-danger);opacity:.7}.cps-action{appearance:none;background:var(--cps-accent);color:#0a0a0c;border:1px solid var(--cps-accent);min-width:220px;height:46px;font-family:var(--cps-mono);text-transform:uppercase;letter-spacing:.24em;cursor:pointer;clip-path:polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,0 100%);outline:none;justify-content:center;align-items:center;gap:12px;margin:22px 0 0;padding:0 22px;font-size:12px;font-weight:700;transition:transform .1s,background .14s,color .14s;display:inline-flex;position:relative}.cps-action:hover:not(:disabled){background:#fff100;transform:translateY(-1px)}.cps-action:active:not(:disabled){background:var(--cps-accent-deep);transform:translateY(0)}.cps-action:focus-visible{outline:2px solid var(--cps-accent);outline-offset:3px}.cps-action:disabled{background:var(--cps-surface-2);color:var(--cps-ink-dim);border-color:var(--cps-line-bright);cursor:progress}.cps-action-glyph{justify-content:center;align-items:center;width:14px;height:14px;font-size:12px;line-height:1;display:inline-flex}.cps-action:disabled .cps-action-glyph{color:var(--cps-accent);animation:1.6s linear infinite cps-spin}@keyframes cps-spin{to{transform:rotate(360deg)}}.cps-restore{appearance:none;height:46px;color:var(--cps-ink-dim);border:1px solid var(--cps-line-bright);font-family:var(--cps-mono);text-transform:uppercase;letter-spacing:.22em;cursor:pointer;background:0 0;justify-content:center;align-items:center;gap:8px;margin:22px 0 0;padding:0 18px;font-size:11px;font-weight:500;transition:color .14s,border-color .14s,background .14s;display:inline-flex}.cps-restore:hover:not(:disabled){color:var(--cps-ink);border-color:var(--cps-ink-dim);background:#ffffff05}.cps-restore:disabled{color:var(--cps-ink-fade);border-color:var(--cps-line);cursor:not-allowed}.cps-restore-glyph{justify-content:center;align-items:center;font-size:14px;display:inline-flex;transform:translateY(-1px)}.cps-status{flex-direction:column;justify-content:flex-end;gap:6px;min-width:0;padding-left:4px;display:flex}.cps-status-head{letter-spacing:.28em;text-transform:uppercase;color:var(--cps-ink-dim);align-items:center;gap:10px;font-size:10px;display:flex}.cps-led{background:var(--cps-line-bright);border-radius:50%;flex-shrink:0;width:9px;height:9px;position:relative;box-shadow:inset 0 0 0 1px #00000073}.cps-led:after{content:\"\";background:0 0;border-radius:50%;position:absolute;inset:-3px}.cps-led[data-kind=loading]{background:var(--cps-accent);animation:1.1s ease-in-out infinite cps-led-pulse}.cps-led[data-kind=done],.cps-led[data-kind=restored]{background:var(--cps-accent);box-shadow:0 0 0 1px #00000073,0 0 6px #ffd60a80}.cps-led[data-kind=err]{background:var(--cps-danger);--cps-pulse-color:#ff636366;animation:.9s ease-in-out infinite cps-led-pulse;box-shadow:0 0 0 1px #00000073,0 0 6px #ff63638c}@keyframes cps-led-pulse{0%,to{box-shadow:0 0 0 0 var(--cps-pulse-color,#ffd60a8c)}50%{box-shadow:0 0 0 5px #0000}}.cps-status-body{background:var(--cps-bg-deep);border:1px solid var(--cps-line);font-family:var(--cps-mono);color:var(--cps-ink);white-space:nowrap;text-overflow:ellipsis;align-items:center;gap:8px;padding:10px 12px;font-size:12px;display:flex;overflow:hidden}.cps-status-prompt{color:var(--cps-accent);flex-shrink:0;font-weight:700}.cps-status-msg{font-variant-numeric:tabular-nums;text-overflow:ellipsis;min-width:0;overflow:hidden}.cps-status-msg[data-kind=err]{color:var(--cps-danger)}.cps-status-msg[data-kind=loading]{color:var(--cps-ink)}.cps-status-msg[data-kind=done]{color:var(--cps-good)}.cps-status-msg[data-kind=restored]{color:var(--cps-info)}.cps-status-msg[data-kind=idle]{color:var(--cps-ink-dim)}.cps-status-cursor{background:var(--cps-accent);vertical-align:middle;width:7px;height:13px;margin-left:2px;animation:.95s step-end infinite cps-cursor;display:inline-block}@keyframes cps-cursor{0%,49%{opacity:1}50%,to{opacity:0}}.cps-progress{background:var(--cps-bg-deep);border:1px solid var(--cps-line);height:4px;position:relative;overflow:hidden}.cps-progress-fill{background:linear-gradient(90deg, var(--cps-accent-deep), var(--cps-accent));transition:width .22s;position:absolute;top:0;bottom:0;left:0}.cps-progress-ticks{pointer-events:none;background-image:repeating-linear-gradient(90deg,#0000 0 calc(10% - 1px),#00000059 calc(10% - 1px) 10%);position:absolute;inset:0}.cps-stats{border-top:1px dashed var(--cps-line);grid-template-columns:repeat(4,1fr);gap:14px;margin-top:16px;padding-top:14px;animation:.46s cubic-bezier(.2,.8,.2,1) both cps-stats-in;display:grid}@media (width<=760px){.cps-stats{grid-template-columns:repeat(2,1fr)}}@keyframes cps-stats-in{0%{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}.cps-stat{background:var(--cps-surface);border:1px solid var(--cps-line);flex-direction:column;gap:4px;padding:10px 14px;animation:.52s cubic-bezier(.2,.8,.2,1) both cps-stats-in;display:flex;position:relative}.cps-stat:before{content:\"\";background:var(--cps-accent);width:18px;height:1px;position:absolute;top:0;left:0}.cps-stat:first-child{animation-delay:60ms}.cps-stat:nth-child(2){animation-delay:.12s}.cps-stat:nth-child(3){animation-delay:.18s}.cps-stat:nth-child(4){animation-delay:.24s}.cps-stat-label{letter-spacing:.28em;text-transform:uppercase;color:var(--cps-ink-dim);font-size:9.5px}.cps-stat-value{font-family:var(--cps-mono);color:var(--cps-ink);font-variant-numeric:tabular-nums;letter-spacing:.01em;align-items:baseline;gap:6px;font-size:22px;font-weight:700;line-height:1.1;display:flex}.cps-stat-value[data-accent=true]{color:var(--cps-accent)}.cps-stat-value[data-dim=true]{color:var(--cps-ink-dim)}.cps-stat-unit{letter-spacing:.18em;text-transform:uppercase;color:var(--cps-ink-dim);font-size:10px;font-weight:500}.cps-stat-sub{letter-spacing:.22em;text-transform:uppercase;color:var(--cps-ink-fade);font-size:9.5px}.lzt-cps-ratio-badge{--cps-tag-bg:#ffd60a;--cps-tag-ink:#0a0a0c;--cps-tag-line:#b59500;background:var(--cps-tag-bg);width:max-content;max-width:100%;color:var(--cps-tag-ink);letter-spacing:.08em;text-transform:uppercase;clip-path:polygon(0 0,calc(100% - 8px) 0,100% 50%,calc(100% - 8px) 100%,0 100%);pointer-events:none;vertical-align:middle;align-self:flex-start;align-items:center;gap:10px;margin:8px 8px 4px 0;padding:5px 14px 5px 10px;font-family:JetBrains Mono,ui-monospace,SF Mono,Menlo,monospace;font-size:11px;font-weight:500;line-height:1;display:inline-flex;position:relative}.lzt-cps-ratio-badge:before{content:\"\";background:var(--cps-tag-ink);opacity:.55;border-radius:50%;flex-shrink:0;width:6px;height:6px;display:inline-block}.lzt-cps-ratio-badge-key{letter-spacing:.22em;font-weight:700}.lzt-cps-ratio-badge-value{letter-spacing:.02em;text-transform:none;font-variant-numeric:tabular-nums;font-size:14px;font-weight:700}.lzt-cps-ratio-badge-meta{letter-spacing:.12em;opacity:.75;text-transform:none;font-size:10.5px;font-weight:500}.lzt-cps-ratio-badge[data-empty=true]{--cps-tag-bg:#2a2a30;--cps-tag-ink:#8a857a;--cps-tag-line:#3a3a44}.lzt-cps-ratio-badge[data-empty=true]:before{opacity:.4}");
	...
	var DEFAULT_PAGES = 5;
	var MAX_PAGES = 50;
	var SOFT_WARN_PAGES = 20;
	function statusText(s) {
		switch (s.kind) {
			case "idle": return "standby — input range and execute";
			case "loading": return `scanning page ${s.done.toString().padStart(2, "0")} of ${s.total.toString().padStart(2, "0")}`;
			case "done": {
			...
			case "err": return `error — ${s.msg}`;
			case "restored": return "restored to original ordering";
		}
	}
	function Clock() {
		const [now, setNow] = (0, import_react.useState)(() => new Date());
		(0, import_react.useEffect)(() => {
			const id = window.setInterval(() => setNow(new Date()), 1e3);
			return () => window.clearInterval(id);
		}, []);
		return (0, import_jsx_runtime.jsx)("span", {
			className: "cps-header-clock",
	...
	function App() {
		const [pages, setPages] = (0, import_react.useState)(() => loadNumber(PAGES_KEY, DEFAULT_PAGES));
		...
		const onRestore = (0, import_react.useCallback)(() => {
			abortRef.current?.abort();
			cleanupDecorations();
		...
		const progress = status.kind === "loading" && status.total > 0 ? Math.round(status.done / status.total * 100) : 0;
		return (0, import_jsx_runtime.jsxs)("div", {
			className: "lzt-cps-panel",
			role: "region",
			"aria-label": "Cost-per-champion sort",
			children: [
				(0, import_jsx_runtime.jsxs)("header", {
					className: "cps-header",
					children: [(0, import_jsx_runtime.jsxs)("div", {
						className: "cps-header-title",
						children: [
							(0, import_jsx_runtime.jsx)("span", {
								className: "cps-header-mark",
								children: "LZT · CPS"
							}),
							(0, import_jsx_runtime.jsx)("span", {
								className: "cps-header-rule",
								"aria-hidden": "true"
							}),
							(0, import_jsx_runtime.jsx)("span", {
								className: "cps-header-name",
								children: "cost per champion"
							})
						]
					}), (0, import_jsx_runtime.jsxs)("div", {
						className: "cps-header-meta",
						children: [(0, import_jsx_runtime.jsxs)("span", {
							className: "cps-header-mod",
							children: ["mod. ", (0, import_jsx_runtime.jsx)("b", { children: "CPS-01" })]
						}), (0, import_jsx_runtime.jsx)(Clock, {})]
					})]
				}),
				(0, import_jsx_runtime.jsxs)("div", {
					className: "cps-controls",
					children: [
						(0, import_jsx_runtime.jsxs)("div", {
							className: "cps-stepper",
							children: [
								(0, import_jsx_runtime.jsx)("label", {
									className: "cps-stepper-label",
									htmlFor: "cps-pages-input",
									children: "page range"
								}),
								(0, import_jsx_runtime.jsxs)("div", {
									className: "cps-stepper-body",
									children: [
								...
								(0, import_jsx_runtime.jsxs)("div", {
									className: "cps-stepper-meta",
									"data-warn": warn || void 0,
									children: [(0, import_jsx_runtime.jsx)("span", { children: warn ? "caution · heavy fetch" : "pages to aggregate" }), (0, import_jsx_runtime.jsxs)("span", { children: ["max ", MAX_PAGES.toString().padStart(2, "0")] })]
								})
							]
						}),
						(0, import_jsx_runtime.jsxs)("button", {
							...
							children: [(0, import_jsx_runtime.jsx)("span", {
								className: "cps-action-glyph",
								"aria-hidden": "true",
								children: loading ? "◴" : "◆"
							}), (0, import_jsx_runtime.jsx)("span", {
								className: "cps-action-label",
								children: loading ? "scanning" : "execute sort"
							})]
						}),
						(0, import_jsx_runtime.jsxs)("div", {
							...
							children: [
								(0, import_jsx_runtime.jsxs)("div", {
									className: "cps-status-head",
									children: [(0, import_jsx_runtime.jsx)("span", {
										className: "cps-led",
										"data-kind": status.kind,
										"aria-hidden": "true"
									}), (0, import_jsx_runtime.jsxs)("span", { children: ["channel · ", loading ? `${progress.toString().padStart(2, "0")}%` : "idle"] })]
								}),
								(0, import_jsx_runtime.jsxs)("div", {
									className: "cps-status-body",
									"aria-live": "polite",
									children: [(0, import_jsx_runtime.jsx)("span", {
										className: "cps-status-prompt",
										"aria-hidden": "true",
										children: ">"
									}), (0, import_jsx_runtime.jsxs)("span", {
										className: "cps-status-msg",
										"data-kind": status.kind,
										children: [statusText(status), loading && (0, import_jsx_runtime.jsx)("span", {
											className: "cps-status-cursor",
											"aria-hidden": "true"
										})]
									})]
								}),
								loading && (0, import_jsx_runtime.jsxs)("div", {
									className: "cps-progress",
									"aria-hidden": "true",
									children: [(0, import_jsx_runtime.jsx)("div", {
										className: "cps-progress-fill",
										style: { width: `${progress}%` }
									}), (0, import_jsx_runtime.jsx)("div", { className: "cps-progress-ticks" })]
								})
							]
						}),
						canRestore && (0, import_jsx_runtime.jsxs)("button", {
							type: "button",
							className: "cps-restore",
							onClick: onRestore,
							disabled: loading,
							children: [(0, import_jsx_runtime.jsx)("span", {
								className: "cps-restore-glyph",
								"aria-hidden": "true",
								children: "↺"
							}), (0, import_jsx_runtime.jsx)("span", { children: "restore" })]
						})
					]
				}),
				status.kind === "done" && (0, import_jsx_runtime.jsx)(StatsGrid, { outcome: status.outcome })
			]
		});
	}
	function StatsGrid({ outcome }) {
	...
	function Stat({ label, value, unit, sub, accent, dim }) {
		return (0, import_jsx_runtime.jsxs)("div", {
			className: "cps-stat",
			children: [
				...
				(0, import_jsx_runtime.jsxs)("div", {
					...
					"data-dim": dim || void 0,
					children: [(0, import_jsx_runtime.jsx)("span", { children: value }), unit && (0, import_jsx_runtime.jsx)("span", {
						className: "cps-stat-unit",
						children: unit
					})]
				}),
				sub && (0, import_jsx_runtime.jsx)("div", {
					className: "cps-stat-sub",
					children: sub
				})
			]
		});
	}
	var MOUNT_ID = "lzt-cps-mount";
	...
	function findAnchor() {
		...
		return Array.from(document.querySelectorAll("label")).find((l) => /cheap first|expensive first|newest|oldest/i.test(l.textContent ?? ""))?.closest("div, fieldset, form") ?? null;
	}
	function mount(anchor) {
		...
		(0, import_client.createRoot)(host).render((0, import_jsx_runtime.jsx)(import_react.StrictMode, { children: (0, import_jsx_runtime.jsx)(App, {}) }));
	}
	function waitForAnchor(timeoutMs = 1e4) {
